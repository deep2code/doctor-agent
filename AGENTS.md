...
- **Zero-config first-run setup (2026-08-09)**: 下载 Release 二进制后双击 `start-chat`（release.yml 每平台附带的一键脚本）即可用。无 API Key 时 `runChat` 触发交互引导（三选一：**智谱 glm-4-flash 免费 / DeepSeek / 豆包(火山方舟)**，火山方舟 `model` 直接用模型名如 `doubao-seed-2-1-pro-260628` 或接入点 ID `ep-xxxx`），把 `LLM_PROVIDER`/key 写入 `~/.doctor-agent/config.env`（0600），`loadUserConfig()` 以最低优先级注入环境变量（真实 env > `.env` > 用户配置）。`runServe` 缺 key 时打印指引。用户侧零技术门槛；开发侧一切照旧。
