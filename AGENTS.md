...
- **鼻咽癌口语关键词 (2026-09-21)**: `data/nasopharyngeal_carcinoma.json` npc-001 补 `鼻塞/回吸性血涕/涕中带血/鼻涕带血/耳闷/颈部肿块`（鼻咽癌经典早期症状，此前只有 耳鸣/鼻出血/颈部淋巴结，导致 `TestRetrieverSymptomStyleChineseRecall` 的 "耳鸣鼻塞回吸性血涕" 被 diseaseenc/msd 同名条目挤出 top5）。已 `make_gz.py` 重压该 gz 并重新 seed。**未决**: `TestRetrieverNoRecallForUnrelated` 的 "我家猫最近不吃东西" 仍失败——检索层无法安全修（见下条），别再调阈值。
- **宠物查询的召回断言放错层 (2026-09-21，待决策)**: `retriever_test.go TestRetrieverNoRecallForUnrelated` 期望检索层对 "我家猫最近不吃东西" 返回 0 条。实测: 该查询字面通道 0 命中，扩展通道经 `alias_map.json` 的 食欲不振 组（不吃→挑食/偏食/厌食）命中 6.0~8.0（阈值 3.0）。**任何分数阈值都救不了它**: 提到 6.0 只杀掉 2 个泛化别名命中的噪声（学龄前膳食指南），杀不掉标题即 `食欲不振` 的 diseaseenc/msd 条目（单一强字段命中 ≥7.0）；而合法的口语儿科查询 "突然大哭→夜惊/绞痛" 结构与之一模一样（纯别名命中、字面 0 命中、命中数更少），提高阈值必然伤害真召回。区分二者只有「猫=动物」这类世界知识，而它已在正确的层实现: `safety.ScopeGuard` 的 veterinary 规则（`internal/safety/scope_guard.go:38`，关键词 宠物/猫/狗/兽医/动物，`medicalOverrideKeywords` 保护 咬/抓/狂犬病 等人类暴露问题不被误封），且 `scope_guard_test.go` 已断言 "我的猫生病了吃什么药" 被拒。生产管线顺序是 L1→**L2 scope guard**→检索，故该查询在检索前就被拒，端到端行为本来就对。**建议（未执行，等产品决策）**: 把这条断言从 knowledge 层移到 safety 层，而不是放宽/删除测试。
