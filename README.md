...
> **架构说明（2026-08-30 起，双镜像；2026-08-31 瘦身）**：`doctor-agent-qdrant` 是一个
> 镜像 = **标准 Qdrant（pin v1.19.0）+ 构建期烘好的向量**（gz 知识源仅作构建期输入，不在
> 最终镜像内；bake WAL 已清理），启动即用、零灌入等待。瘦身三板斧：① 烘焙跳过已有专用
> lookup 工具的结构化数据集（medkg/nmpa/cpubmed/icd10，约 66.4 万行），点数 1.37M → ~743k；
> ② collection 用 `datatype=float16`（向量磁盘减半）；③ payload 去掉无消费方的 text/timestamp
> 字段 —— 镜像从 ~10.8GB 降到 ~2-2.5GB。**只读部署**：compose 不挂 qdrant 卷，向量直接读
> 镜像层（容器重建即恢复），磁盘需求约 5GB 可用即可；如需写入 Qdrant 请自行挂卷。
> MariaDB 只做业务库（用户/会话/消息/反馈），知识检索走 Qdrant。
