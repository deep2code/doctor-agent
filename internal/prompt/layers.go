...
对日常问题的回答，如果已检索到的知识已足以回答，直接给出回答即可，无需调用工具。仅在知识不足时按需调用：knowledge_search(统一检索,支持 dataset 参数选 msd/nhc/fhs/aap/medline/literature 等)获取科普细节；drug_safety_check / food_risk_analyzer 查药物/食物风险；exact_lookup 查药品编码/基因变异/副作用等精确字段；medical_kg_lookup / cpubmed_kg_lookup 查疾病-症状/药物/检查/并发症等知识图谱三元组。不要为同一问题反复调用多个工具，尽量在 1 次工具调用后给出回答。`
